...
function ihttp_request($url, $post = '', $extra = [], $timeout = 60)
{
...
function ihttp_multi_request($urls, $posts = [], $extra = [], $timeout = 60)
{
    if ( ! is_array($urls)) {
...
function ihttp_build_curl($url, $post, $extra, $timeout)
{
    ...
    curl_setopt($ch, CURLOPT_URL,
        $urlset['scheme'] . '://' . $urlset['host'] . ($urlset['port'] == '80' || empty($urlset['port']) ? '' : ':' . $urlset['port']) . $urlset['path'] . $urlset['query']);
